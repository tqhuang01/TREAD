...
class MIMIC(object):
    ...
    def __init__(self, root, n_samples=None, device=torch.device("cpu")):
        ...
        if device == torch.device("cpu"):
            ...
            self.data = torch.load(os.path.join(self.processed_folder, 'mimic.pt'), weights_only=False)
    ...
    def __init__(self, in_df, validation=False, val_options=None):
        ...
        if self.validation:
            ...
            if val_options.get("T_stop"):
                after_idx = self.df.loc[
                    (self.df['Time'] > val_options['T_val']) & (self.df['Time'] < val_options['T_stop']), 'ID'].unique()
            else:
        ...
        if self.validation:
            ...
            if val_options.get("T_stop"):
                self.df_after = self.df_after.loc[self.df_after['Time'] < val_options['T_stop']].sort_values(
                    'Time').copy()
