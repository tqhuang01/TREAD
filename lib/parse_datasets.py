...
def parse_datasets(args, patch_ts=False, length_stat=False):
    ...
    if dataset_name in ["physionet", "mimic"]:
        ...
        batch_size = min(min(len(seen_data), args.batch_size), args.num_samples)
    ...
    ### USHCN dataset ###
        ...
        ### list of tuples (record_id, tt, vals, mask) ###
        total_dataset = USHCN('./data/ushcn/', n_samples=args.num_samples, device=device)
    ...
    ### Activity dataset ###
        ...
        total_dataset = PersonActivity('./data/activity/',
                                       download=True,
                                       n_samples=args.num_samples,
                                       device=device)
